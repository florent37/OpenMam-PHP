...
class AppRepositoryDb implements AppRepository {
    ...
    {
        $db = getConnection();
        $requete=$db->prepare( "SELECT a.*, max(v.CODE) as lastCode, v.version as lastVersion, v.DATE as dateLastUpdate FROM APPLICATION a, VERSION v WHERE a.APP_ID = v.APP_ID"); //verify user access, for now give him all apps
        $requete->execute();
